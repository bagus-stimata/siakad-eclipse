 ...
 * @author yhawin
 */
public class MatkulKelompok extends AbstractValidator{
    //Inisialisasi Dao
    ...
    private List<MMatkulKel> listMatkulKelompok;
    private MMatkulKel matkulKel;
    private MMatkulKel tempMatkulKel; //temp digunakan untuk: Tambah Baru
    ...
    public void init(){
        ...
        listMatkulKelompok = matkulKelDao.getAll();
        matkulKel = new MMatkulKel();
        //Jika ada isinya maka isi dengan yang pertama jika tidak ada biarin kosong
        if (listMatkulKelompok.size() > 0)
            matkulKel = listMatkulKelompok.get(0);
    ...
    @NotifyChange({"matkulKel", "addNew", "resetInputIcon"})
    @Command("baru")
    public void baru(){
        setTempMatkulKel(getMatkulKel());
        matkulKel = new MMatkulKel();
        setResetInputIcon(true);
        setAddNew(true);
    }
    
    //@NotifyChange({"listMatkulKelompok", "matkulKel"}) //Notify Change ini tidak berlaku jika pake MessageBox sehingga NotifyChange harus pake BindUtils.NotifyChange
    @Command("hapus") 
    public void hapus(){
                ...
                new org.zkoss.zk.ui.event.EventListener(){
                    ...
                    public void onEvent(Event e){
                        ...
                        //OK is clicked
                            ...
                            if (matkulKel.getMatkulSet().size()>0) {       
                                  Messagebox.show("Kode Kelompok tersebut masih dipakai oleh tabel Mata Kuliah", "Warning", 
                                          Messagebox.OK, Messagebox.EXCLAMATION);
                            } else {
                                //Hapus didatabase
                                matkulKelDao.delete(matkulKel);
                                //Hapus di tabel 
                                listMatkulKelompok.remove(matkulKel); //Notify Change gak berhasil disini
                                //Kosongkan isi teksbox
                                matkulKel = new MMatkulKel();
                                //Karena pake notifyChange Annotiotion Biasa tidak bisa maka pake cara ini BindUtils.postNotifyChange
                                BindUtils.postNotifyChange(null, null, MatkulKelompok.this, "listMatkulKelompok");        
                                BindUtils.postNotifyChange(null, null, MatkulKelompok.this, "matkulKel");        
    ...
    @NotifyChange({"listMatkulKelompok", "addNew", "resetInputIcon"})
    @Command("simpan")
    public void simpan(){
        //simpan di database
        matkulKelDao.saveOrUpdate(matkulKel);
        //Jika tombol tambah maka list(tabel) ditambah isinya
        if (isAddNew()==true)
            listMatkulKelompok.add(matkulKel);
    ...
    public void validate(ValidationContext vc) {
        ...
        validateKode(vc, (String)beanProps.get("idKelompok").getValue(), (Boolean)vc.getValidatorArg("tambahBaru"));        
    ...
    private void validateKode(ValidationContext ctx, String kode, boolean tambahBaru) {
        ...
        if(kode.trim().equals("")) {
            ...
            //1. Cek Kode
            if (matkulKelDao.findById(kode) != null) {
                MMatkulKel temp = new MMatkulKel();
                temp = matkulKelDao.findById(kode);
    ...
    public List<MMatkulKel> getListMatkulKelompok() {
        return listMatkulKelompok;
    }

    public void setListMatkulKelompok(List<MMatkulKel> listMatkulKelompok) {
        this.listMatkulKelompok = listMatkulKelompok;
    }
    ...
    public MMatkulKel getTempMatkulKel() {
        return tempMatkulKel;
    }

    public void setTempMatkulKel(MMatkulKel tempMatkulKel) {
        this.tempMatkulKel = tempMatkulKel;
    }
